...
default_cfgs = {
    ...
    'vit_base_patch32_224': _cfg(
        ...
        url2 ='https://storage.googleapis.com/vit_models/augreg/'
        'augreg/B_16-i1k-300ep-lr_0.001-aug_light0-wd_0.1-do_0.0-sd_0.0--cifar100-steps_10k-lr_0.01-res_224.npz'
    ),
    ...
    'vit_base_patch16_224': _cfg(
        url2='https://storage.googleapis.com/vit_models/imagenet21k/ViT-B_16.npz'
    ,   url ='https://storage.googleapis.com/vit_models/augreg/'
        'B_16-i21k-300ep-lr_0.001-aug_medium1-wd_0.1-do_0.0-sd_0.0--imagenet2012-steps_20k-lr_0.01-res_224.npz',
        url3 = 'https://storage.googleapis.com/vit_models/augreg/'
    'B_16-i1k-300ep-lr_0.001-aug_light0-wd_0.1-do_0.0-sd_0.0--cifar100-steps_10k-lr_0.01-res_224.npz'),          #换了一个url
    ...
    'test_cifar100': _cfg(url='https://storage.googleapis.com/vit_models/augreg/'
                          'B_16-i1k-300ep-lr_0.001-aug_light0-wd_0.03-do_0.0-sd_0.0--cifar100-steps_2k-lr_0.03-res_224.npz')
}


class Prompt(nn.Module):
    def __init__(self, length=5, embed_dim=768, embedding_key='mean', prompt_init='uniform', prompt_pool=False,
                 prompt_key=False, pool_size=None, top_k=None, batchwise_prompt=False, prompt_key_init='uniform',stop_init =False ):  #new一个新变量
        super().__init__()
        # self.stop_init =s
        self.length = length  # prompt length
        self.embed_dim = embed_dim  # embed_dim
        self.prompt_pool = prompt_pool
        self.embedding_key = embedding_key  # The way in which x_embed is calculated
        self.prompt_init = prompt_init  # prompt 初始化
        self.prompt_key = prompt_key  # using learnable prompt keys
        self.pool_size = pool_size  # prompt_pool_size 大小
        self.top_k = top_k  # 选择前K个prompt
        self.batchwise_prompt = batchwise_prompt  # prompt是否以batch为单位


        if self.prompt_pool:
            self.prompt = None#初始化prompt
            self.prompt_sim = None#初始化prompt_sim
            # prompt_pool_shape = (pool_size, length, embed_dim)  # propmt pool (pool_size, length, embed_dim)
            # if prompt_init == 'zero':  # 初始化prompt_pool
        ...
        #     if prompt_key_init == 'zero':  # 初始化prompt_key
        #         self.prompt_key = nn.Parameter(torch.zeros(key_shape))
        #     elif prompt_key_init == 'uniform':
        #         self.prompt_key = nn.Parameter(torch.randn(key_shape))  # key_shape = (pool_size, embed_dim)
    ...
    def forward(self, x_embed, prompt_mask=None, cls_features=None):  # x_embed 维度? (B,196,768)
        out = dict()
        if self.prompt_pool:  # 如果使用prompt pool 则计算embedding_mean
            if self.embedding_key == 'mean':
                x_embed_mean = torch.mean(x_embed, dim=1)  # (B,1,768)
            elif self.embedding_key == 'max':
                x_embed_mean = torch.max(x_embed, dim=1)[0]  # (B,1,768)
            ...
            if self.prompt_key is None:               #如果为空添加初始量
                # self.prompt_key = nn.Parameter((torch.randn(1,self.embed_dim))).cuda(3)
                print("type(self.prompt_key.data)")
                self.prompt_key = torch.mean(x_embed_mean,dim=0)
                self.prompt_key =self.prompt_key.reshape(1,768)
                print(self.prompt_key.data.shape)
                self.share_prompt = nn.Parameter(
                    torch.randn(12,2, self.embed_dim, device='cuda:1'))  ###############
                self.deep_prompt = nn.Parameter(torch.randn(15, 12, 2, self.embed_dim, device='cuda:1'))
                # self.share_prompt = nn.Parameter(self.share_prompt.expand(50,-1,-1).clone())
                self.prompt = nn.Parameter(torch.randn(1,self.length,self.embed_dim,device='cuda:1'))
                self.prompt_sim = torch.tensor([0],dtype=torch.float32 ,device='cuda:1') #相似度添加
                # self.prompt_sim = None
                # print(self.prompt)
                prompt_norm = self.l2_normalize(self.prompt_key, dim=1)  # Pool_size, C
                x_embed_norm = self.l2_normalize(x_embed_mean, dim=1)  # B, C
                similarity = torch.matmul(x_embed_norm, prompt_norm.t())
                major_prompt_id = torch.tensor([[self.prompt_key.shape[0] - 1]])
                idx = major_prompt_id.expand(x_embed.shape[0], -1)
                # self.prompt = torch.tensor(self.prompt)
                # self.prompt_key = torch.tensor(self.prompt_key)
                # self.sim_top_k_sum =torch.tensor([0],device='cuda:2')   #初始化一个sim_top_k_sum
            else:

                prompt_norm = self.l2_normalize(self.prompt_key, dim=1)  # Pool_size, C
                # print(prompt_norm.shape)
                # prompt_norm = prompt_norm.cuda(3)
                x_embed_norm = self.l2_normalize(x_embed_mean, dim=1)  # B, C

                similarity = torch.matmul(x_embed_norm, prompt_norm.t())  # B, Pool_size       每一个样本对所有prompt的相似度
                ...
                sim_top_k, idx = torch.topk(similarity, k=self.top_k, dim=1)  # B, top_k         #每一个样本选出k最大相似度的prompt
                prompt_id, id_counts = torch.unique(idx, return_counts=True, sorted=True)  # 做集合处理
                # In jnp.unique, when the 'size' is specified and there are fewer than the indicated number of elements,
                # the remaining elements will be filled with 'fill_value', the default is the minimum value along the specified dimension.
                # Unless dimension is specified, this will be flattend if it is not already 1D.
                # print(id_counts,self.top_k)
                _, major_idx = torch.topk(id_counts, k=self.top_k)  # top_k
                major_prompt_id = prompt_id[major_idx]  # top_k
                similarity_chosen = similarity[:,major_prompt_id]
                # print(similarity_chosen.sum(),major_prompt_id)

                # self.prompt_sim[major_prompt_id] = similarity_chosen.sum()
                # self.sim_top_k_sum =sim_top_k[:, self.top_k - 1].sum()
                # print("1111111111111111111111111111111111111111111111")
                # print(sim_top_k)
                # print(sim_top_k.shape)
                # print(sim_top_k[:,self.top_k-1])
                # print(sim_top_k[:, self.top_k - 1].sum())            #####查看sim_topk_sum
                # print(similarity_chosen.sum()/len(x_embed))
                if((similarity_chosen.sum()/len(x_embed)<=0.2)):       #c 0.2
                    self.prompt_key.data =torch.cat([(self.prompt_key).data,(torch.mean(x_embed_mean,dim=0).reshape(1,self.embed_dim)).cuda(1)],dim=0)
                    # self.prompt_key = nn.Parameter(self.prompt_key)
                    self.prompt.data = torch.cat([self.prompt.data,(torch.randn(1,self.length,self.embed_dim)).cuda(1)],dim=0)
                    if(self.prompt_sim[0]==0):  #将刚开始的0值覆盖
                        self.prompt_sim[0]=similarity_chosen.sum()
                    self.prompt_sim =torch.cat([self.prompt_sim,torch.tensor([similarity_chosen.sum()]).cuda(1)],dim=0)    #可改变初始化的方式
                    # self.prompt_sim = torch.cat([self.prompt_sim, torch.tensor([torch.mean(self.prompt_sim).data]).cuda(1)],dim=0)
                    major_prompt_id = torch.tensor([[self.prompt_key.shape[0] - 1]])
                self.prompt_sim[major_prompt_id] = 0.1*float(similarity_chosen.sum())+0.9*float(self.prompt_sim[major_prompt_id])
                # print(self.prompt_sim)
                # print('self.prompt_sim[major_prompt_id] is       ',self.prompt_sim[major_prompt_id])
                idx = major_prompt_id.expand(x_embed.shape[0], -1)
            self.prompt_idx= idx
            # similarity_k = itemgetter((idx),(similarity))                 #取出K的元素的相似度
            # similarity_k_1 = [similarity[i]for i in idx]
            ...
            # batched_prompt_raw = torch.cat([batched_prompt_raw,self.share_prompt],dim=2) #######   share cat all
            ...
            # print("bbbbbatch_key_norm is")
            # print(batched_key_norm)
        ...
        # batched_prompt.cuda(3)
        # print('batched_prompt.shape')              #查看相应变量
        # print(batched_prompt.shape)
        # print('x_embed.shape')
        # print(x_embed.shape)
        # The input with the prompt concatenated to the front. [B, prompt+token, C]
        out['total_prompt_len'] = batched_prompt.shape[1]
        # batched_prompt =batched_prompt.cuda(3)
        out['share_prompt'] =self.share_prompt
        out['deep_prompt'] = self.deep_prompt
        out['prompted_embedding'] = torch.cat([batched_prompt, x_embed], dim=1)
        # print('self.prompt_key[major_prompt_id]')
        x_embed_all_mean = torch.mean(x_embed_mean,dim=0)
        self.prompt_key[major_prompt_id,]=0.8*self.prompt_key[major_prompt_id,]+0.2*x_embed_all_mean

        return out

class Attention(nn.Module): #vit组成部分attention
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            self, img_size=224, patch_size=16, in_chans=3, num_classes=10, global_pool='token',
            ...
            top_k=None, batchwise_prompt=False, prompt_key_init='uniform', head_type='token', use_prompt_mask=False,deep_prompt = False):
        """
        Args:
        ...
        self.patch_embed = embed_layer(
            img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim)    #(B,3,224,224)->(B,196,768)
    ...
    def forward_features(self, x,  cls_features=None, index = 0,train=False):
        x = self.patch_embed(x)     #(B,3,224,224)->(B,196,768)

            # print(x.shape,Tprompt.shape)
        if hasattr(self, 'prompt'): #查看是否存在prompt
            res = self.prompt(x, cls_features=cls_features)
            # self.prompt.prompt_key=nn.Parameter(self.prompt.prompt_key)          #
            # self.prompt.prompt =nn.Parameter(self.prompt.prompt)                 #
            self.total_prompt_len = res['total_prompt_len']
            x = res['prompted_embedding']
            deep_prompt = res['deep_prompt']
            idx = res['prompt_idx']
            # print(idx.shape)
            # print(idx)
            # share_prompt = nn.Parameter(share_prompt.expand(50,-1,-1).clone())
            # print(deep_prompt[idx[0]].shape)
            self.deep_prompt_select = deep_prompt[idx].reshape(len(idx),12,2,768)
            share_prompt = res['share_prompt']
            self.share_prompt = share_prompt.expand(x.shape[0],-1,-1,-1)
            # print(x.shape)
        else:
            res=dict()
        if self.cls_token is not None:
            x = torch.cat((self.cls_token.expand(x.shape[0], -1, -1), x), dim=1) #cls token拼接在x前
        
        x = self.pos_drop(x + self.pos_embed)
        ################share prompt
        # if hasattr(self, 'prompt'):
        #     # print(res['share_prompt'])
        #     share_prompt = res['share_prompt']
        #     share_prompt = nn.Parameter(share_prompt.expand(50,-1,-1).clone())
        #     x = torch.cat([x,share_prompt],dim=1)
        if hasattr(self, 'prompt'):
            if self.deep_prompt:
                for i in range(12):
                    if i in [3,4,5]:
                        x = torch.cat([self.deep_prompt_select[:, i].reshape(len(x), 2, 768),x], dim=1)
                        # x = torch.cat([self.share_prompt[:,i].reshape(50, 2, 768),x],dim=1)
                        x = self.blocks[i](x)
                    else:
                        x = self.blocks[i](x)
            else:
    ...
    def forward_head(self, res, pre_logits: bool = False):
        ...
        res['logits'] = self.head(x)  #(embed_dim, num_classes)
        
        return res

    def forward(self, x, cls_features=None, train=False,index = 0):
        # if self.prompt_pool and self.pos_embed.shape[1]==204:           #####204   199
        #     fill_tensor = nn.Parameter(torch.rand(1, 3, 768).cuda(1)) ###### fill pos_embed
        #     self.pos_embed = nn.Parameter(torch.cat([self.pos_embed,fill_tensor],dim=1))
        res = self.forward_features(x, cls_features=cls_features, train=train)
        # res['prompt_key'] = 0.9*res['prompt_key']+0.1*res['']
...
def vit_base_patch16_18x2_224(pretrained=False, **kwargs):
    ...
    return model
@register_model
def test_cifar100(pretrained=False, **kwargs):
    model_kwargs = dict(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, init_values=1e-5, block_fn=ParallelBlock, **kwargs)
    model = _create_vision_transformer('test_cifar100', pretrained=pretrained, **model_kwargs)
    return model
