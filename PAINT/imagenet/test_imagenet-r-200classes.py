...
for name in name_list:
    ...
    # print(labels_trans)
        ...
        aug = AugMix(severity=0)
        data_batch_aug = None
        for i in range(len(data)):

            data_pil = to_pil(data[i])
            data_aug = aug.augment_and_mix(images=data_pil, preprocess=preprocess)
            data_aug = data_aug.reshape(1,3,224,224)

            if data_batch_aug ==None:
                data_batch_aug = data_aug
            else:
                data_batch_aug = torch.cat([data_batch_aug,data_aug],dim=0)

        data_batch_aug = data_batch_aug.cuda(1)
        # data_cotta = transforms_cotta(data)
        # print('11111')
        # print(data_batch_aug.shape)
        ...
        loss_2 = nn.CrossEntropyLoss()((torch.mean(softmax_out, dim=0)).reshape(1, 200),
        ...
        # print(softmax_out_aug.shape,sample_list_aug,y_batch_aug['logits'].shape)
        # loss_5 = softmax_entropy(softmax_out_aug[sample_list_aug], one_hot[sample_list_aug]).mean(0)
        # loss_6 = nn.CrossEntropyLoss()(softmax_out_aug[sample_list_aug], one_hot[sample_list_aug])
        # print(loss_3,loss_2,loss_6,sample_list_aug)
        loss = loss_7-loss_2+0.1*loss_3
       # loss = loss_4
