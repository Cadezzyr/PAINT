...
class Prompt(nn.Module):
    def __init__(self, length=5, embed_dim=768, embedding_key='mean', prompt_init='uniform', prompt_pool=False,
        ...
        if self.prompt_pool:
            ...
            self.share_prompt =None # 初始化share_prompt
    ...
    def forward(self, x_embed, prompt_mask=None, cls_features=None,only_test = True):  # x_embed 维度? (B,196,768)
        ...
        if self.prompt_pool:  # 如果使用prompt pool 则计算embedding_mean
            ...
            if self.prompt_key is None:               #如果为空添加初始量
                ...
                self.share_prompt = nn.Parameter(
                    torch.randn(1, self.embed_dim, device='cuda:1'))  ###############
                ...
                self.prompt_sim = torch.tensor([0], device='cuda:1') #相似度添加
                ...
                # if((similarity_chosen.sum()-self.prompt_sim[major_prompt_id].sum()<=-5)):            #cifar10:-5   cifar100:-10
                if only_test==False:

                    if(similarity_chosen.sum()/len(x_embed)<= 0.5):              #25
                        # print('生成新prompt，总个数为'+str(len(self.prompt_sim)+1))
                        ...
                        self.prompt_sim =torch.cat([self.prompt_sim,torch.tensor([similarity_chosen.sum()]).cuda(1)],dim=0)
                        major_prompt_id = torch.tensor([[self.prompt_key.shape[0] - 1]])
            ...
            # batched_prompt_raw = torch.cat([self.share_prompt,batched_prompt_raw],dim=2) #######   share cat all
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        # self.share_prompt = None   ##########share prompt
    ...
    def forward_features(self, x,  cls_features=None, train=False,only_test= True):
        x = self.patch_embed(x)     #(B,3,224,224)->(B,196,768)

        if hasattr(self, 'prompt'): #查看是否存在prompt
            res = self.prompt(x, cls_features=cls_features,only_test=only_test)
            ...
            self.deep_prompt_select = deep_prompt[idx].reshape(x.shape[0],12,2,768)
        ...
        if hasattr(self, 'prompt'):
            if self.deep_prompt:
                for i in range(12):
                    if i in [3, 4, 5, 6, 7]:
                        x = torch.cat([x, self.deep_prompt_select[:, i].reshape(50, 2, 768)], dim=1)
                        x = self.blocks[i](x)
                    else:
                        x = self.blocks[i][x]
            else:
                ...
                x = self.blocks(x)
            # for i in range(12):
            #     # print(self.deep_prompt_select.shape)
            #     if i in[3,4,5,6,7]:
            #
            #         x = torch.cat([x, self.deep_prompt_select[:,i].reshape(50,6,768)], dim=1)
            #         x = self.blocks[i](x)
            #     else:
            #         x = self.blocks[i](x)
                # print(i)
    ...
    def forward(self, x, cls_features=None, train=False,only_test = False):
        # if self.share_prompt == None:
        #     self.share_prompt = nn.Parameter(torch.zeros(1,3, self.embed_dim)).cuda(1)
        #     self.share_prompt = self.share_prompt.expand(40, 3,768)
        # print("11111111")
        # print(self.pos_embed.shape)

        # if self.prompt_pool and self.pos_embed.shape[1]==204:           #####204   199
        #     fill_tensor = nn.Parameter(torch.rand(1, 1, 768).cuda(1)) ###### fill pos_embed
        #     self.pos_embed = nn.Parameter(torch.cat([self.pos_embed,fill_tensor],dim=1))
        res = self.forward_features(x, cls_features=cls_features, train=train,only_test=only_test)
