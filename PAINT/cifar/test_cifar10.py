    ...
    for _ in range(1):
        for corruption in corruptions:
            for severity in severitys:
                ...
                print((acc))
            # new_list = []
            # for i in range(int(corruptions.index(corruption)+1)):
            #     new_list.append(corruptions[i])
            # for test_c in new_list :
            #     print(test_c)
            #     x_test,y_test =load_cifar10c(n_examples=10000,corruptions=[test_c],severity=5)
            #     x_test,y_test= x_test.cuda(1),y_test.cuda(1)
            #     test_len = int(len(y_test)/50)
            #     acc=0
            ...
            #         cls_features = original_model(transform(x_test_batch))
            #         cls_features = cls_features['pre_logits']
            #         y_batch = model(transform(x_test_batch), cls_features=cls_features,only_test=True)
            #         acc += (y_batch['logits'].max(1)[1] == y_test_batch).float().sum()
            #     acc=acc.item() / x_test.shape[0]
            #     print(acc)
